...
import controller.EditarReservaController;
...
public class EditarReservas extends JFrame {
	...
	private EditarReservaController editarReservaController;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					EditarReservas frame = new EditarReservas(0L, new Date().from(Instant.now()),new Date().from(Instant.now()), "Cartão de Crédito");
	 ...
	 * Create the frame.
	 */
	public EditarReservas(Long id, Date in, Date out, String forma) {
		super("Editar Reserva");
		editarReservaController = new EditarReservaController();
		...
		txtDataE.getCalendarButton().setIcon(new ImageIcon(EditarReservas.class.getResource("/imagenes/icon-reservas.png")));
		...
		txtDataS.getCalendarButton().setIcon(new ImageIcon(EditarReservas.class.getResource("/imagenes/icon-reservas.png")));
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				try {
					...
					if (!editarReservaController.validaData(dataEntrada, dataSaida)) {
					...
					txtValor.setText(editarReservaController.handlePreco(dias).toString());
		...
		for(int i=0;i<txtFormaPagamento.getModel().getSize();i++) {
			if (txtFormaPagamento.getModel().getElementAt(i) == forma) {
				txtFormaPagamento.setSelectedIndex(i);
			}
		}
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					if (EditarReservas.txtDataE.getDate() == null || EditarReservas.txtDataS.getDate() == null) {
					...
					editarReservaController.update(id,
							txtDataE.getDate(), txtDataS.getDate(),
							editarReservaController.handlePreco(dias),
